...
export interface ActivityHeatmapOptions {
  /**
   * Optional per-day token baseline used to compute the level thresholds.
   * When filtering (e.g. a source filter), pass the UNfiltered token values so
   * colors keep their global meaning: filtering shrinks what's visible but
   * does not re-scale the color scale, avoiding "remaining sources look bigger
   * than they are" visual distortion.
   */
  levelBaseline?: DailyInput[];
}
...
export function buildActivityHeatmap(
  ...
  // Threshold baseline: the unfiltered values when provided (stable color
  // scale across filter changes), otherwise the values being displayed.
  const baselineRows = options?.levelBaseline ?? dailyRows;
  const baselineByDay = new Map<string, number>();
  for (const row of baselineRows) {
    baselineByDay.set(row.date, Math.max(0, row.tokens));
  }
  ...
  for (let i = 0; i < totalDays; i++) {
    ...
    const value = baselineByDay.get(key) ?? valuesByDay.get(key)?.value ?? 0;
